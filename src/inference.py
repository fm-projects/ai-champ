...
from tqdm import tqdm # Для красивой полосы загрузки
...
def apply_heuristics(text):
    """
    Здесь применяются 100% работающие правила (Регулярки) из нашего EDA.
    Возвращает список кортежей (start, end, category).
    """
    entities = []
    
    # 1. Гипотеза по длинам (Номера, ПИН-коды и т.д.)
    hypothesis_classes = {
        14: "СНИЛС клиента"
    }
    
    pattern_nums = re.compile(r'(?<![\w\-])([\d\s\-]+)(?![\w\-])')
    for match in pattern_nums.finditer(text):
        found_str = match.group(1)
        length = len(found_str)
        if length in hypothesis_classes:
            entities.append((match.start(1), match.end(1), hypothesis_classes[length]))
